...
- (void)setupLoadingView
{
    ...
    //设置图片
    [_dimIamge sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://img.meelive.cn/%@", _imageStr]] placeholderImage:[UIImage imageNamed:@"分类"]];
...
- (void)goPlaying {
    ...
    //获取url
    self.url = [NSURL URLWithString:_liveStr];
...
- (void)moviePlayBackStateDidChange:(NSNotification*)notification {
    ...
    switch (_player.playbackState) {
        ...
        case IJKMPMoviePlaybackStateStopped:
            NSLog(@"视频播放停止 %d", (int)_player.playbackState);
            break;
            
        case IJKMPMoviePlaybackStatePlaying:
            NSLog(@"视频播放 %d", (int)_player.playbackState);
            break;
            
        case IJKMPMoviePlaybackStatePaused:
            NSLog(@"视频暂停 %d", (int)_player.playbackState);
            break;
            
        case IJKMPMoviePlaybackStateInterrupted:
            NSLog(@"视频被打断 %d ", (int)_player.playbackState);
            break;
            
        case IJKMPMoviePlaybackStateSeekingForward:
        case IJKMPMoviePlaybackStateSeekingBackward: {
            NSLog(@"搜索视频 %d:", (int)_player.playbackState);
            break;
        }
            
        default: {
            NSLog(@"不知道什么问题,~~~~(>_<)~~~~ %d", (int)_player.playbackState);
            break;
        }
    }
}
