...
-(void)createTextFields
{
    ...
    UIButton *landBtn=[Factory createButtonFrame:CGRectMake(10, self.bgView.frame.size.height+ self.bgView.frame.origin.y+30,self.view.frame.size.width-20, 37) backImageName:nil title:@"完成" titleColor:[UIColor whiteColor]  font:[UIFont systemFontOfSize:17] target:self action:@selector(clickSettingNewPasswdBtn)];
...
-(void)clickSettingNewPasswdBtn
{
    ...
    for(UIViewController *temVC in temArray){
        ...
        if ([temVC isKindOfClass:[TRLoginViewController class]]){
            ...
            userInfo.passwd = self.passward.text;
            [userInfo saveUserInfoToSandox];
            [self.navigationController popToViewController:temVC animated:YES];
        }
    }
    }else{
        [self.view showWarning:@"设置新的密码"];
    }
}
